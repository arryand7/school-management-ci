...
class Site extends Public_Controller
{
     ...
     * Tries staff login first, then user login if staff not found
    ...
    public function auth()
    {
        ...
        // Check for Google Authenticator module
        ...
        // Use login/user login captcha setting
        $is_captcha = $this->captchalib->is_captcha('login') || $this->captchalib->is_captcha('userlogin');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $staff_login_post = array(
                'email' => $username,
                'password' => $password,
            );
            $staff_result = $this->staff_model->checkLogin($staff_login_post);

            if ($staff_result) {
                // Staff found - process admin login
                $this->_process_staff_login($staff_result, $school[0], $data);
                return;
            }
            ...
            // Try user login (student/parent)
            $user_login_post = array(
                'username' => $username,
                'password' => $password,
            );
            $user_result = $this->user_model->checkLogin($user_login_post);

            if (isset($user_result) && !empty($user_result)) {
                // User found - process user login
                $this->_process_user_login($user_result, $school[0], $data);
                return;
            }

            // Neither staff nor user found
            $data['error_message'] = $this->lang->line('invalid_username_or_password');
            if ($is_captcha) {
                $data['captcha_image'] = $this->captchalib->generate_captcha()['image'];
            }
    ...
    /**
     * Process staff/admin login and set session
     */
    private function _process_staff_login($result, $school_settings, &$data)
    {
        ...
        if ($result->is_active) {
            $logusername = ($result->surname != "") ? $result->name . " " . $result->surname : $result->name;
    ...
    /**
     * Process user (student/parent) login and set session
     */
    private function _process_user_login($login_details, $school_settings, &$data)
    {
        $school = $this->setting_model->get();
        ...
        if ($user->is_active == "yes") {
            if ($user->role == "student") {
                ...
                if ($school[0]['parent_panel_login']) {
                    $result = $this->user_model->checkLoginParent(array(
                        'username' => $this->input->post('username'),
                        'password' => $this->input->post('password'),
                    ));
                } else {
