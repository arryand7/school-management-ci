...
class Walkingm extends OnlineAdmission_Controller
{
    ...
    public function success() {
        ...
        $data   = array();
        $responce = base64_decode(isset($_SERVER["QUERY_STRING"]) ? $_SERVER["QUERY_STRING"] : '', true);
        if ($responce === false) {
            log_message('error', 'Walkingm: failed to decode payment response from query string.');
            redirect(base_url("onlineadmission/checkout/paymentfailed/" . $online_data->reference_no));
            return;
        }

        $payment_responce = json_decode($responce);
        if (!is_object($payment_responce) || json_last_error() !== JSON_ERROR_NONE) {
            log_message('error', 'Walkingm: invalid JSON in payment response. ' . json_last_error_msg());
            redirect(base_url("onlineadmission/checkout/paymentfailed/" . $online_data->reference_no));
            return;
        }
        
        if ($responce !== '' && isset($payment_responce->status) && (int)$payment_responce->status === 200) {
            ...
            $gateway_response['payment_mode']   = 'walkingm';
