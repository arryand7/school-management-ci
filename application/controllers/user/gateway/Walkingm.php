...
class Walkingm extends Studentgateway_Controller {
    ...
    public function success() {
        $gresponce = base64_decode(isset($_SERVER["QUERY_STRING"]) ? $_SERVER["QUERY_STRING"] : '', true);
        if ($gresponce === false) {
            log_message('error', 'Walkingm gateway: failed to decode payment response from query string.');
        ...
        if (!is_object($payment_responce) || json_last_error() !== JSON_ERROR_NONE) {
            log_message('error', 'Walkingm gateway: invalid JSON in payment response. ' . json_last_error_msg());
            redirect(base_url('user/gateway/payment/paymentfailed'));
            return;
        }
        
        if ($gresponce !== '' && isset($payment_responce->status) && (int)$payment_responce->status === 200) {
